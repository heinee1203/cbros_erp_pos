...
import { Package, DollarSign, Hash, TrendingUp, Download } from "lucide-react";
import { downloadCSV } from "@/lib/csv-export";
import { cn } from "@/lib/utils";
import { useAuth } from "@/app/auth-context";
import { useSalesByItemQuery, useSalesSummaryQuery, type ReportFilters } from "@/hooks/use-sales-reports";

const CATEGORY_COLORS: Record<string, string> = {
  TIRES: "bg-blue-500/10 text-blue-600",
  LUBRICANTS: "bg-amber-500/10 text-amber-600",
  HARD_PARTS: "bg-slate-500/10 text-slate-600",
  ACCESSORIES: "bg-purple-500/10 text-purple-600",
  LABOR_SERVICES: "bg-emerald-500/10 text-emerald-600",
};
  ...
  return (
    <div className="mx-auto flex h-full max-w-6xl flex-col">
        ...
        {summary && (
          <div className="mt-4 flex gap-5">
            ...
            <div className="flex items-center gap-2 text-[13px]">
              ...
              <span className="font-semibold tabular-nums text-foreground">PHP {fmt(summary.avgTransactionValue)}</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        {(dateFrom || dateTo) && (
          <button onClick={clearDates} className="h-8 rounded-lg border border-border px-3 text-[11px] font-medium text-muted-foreground hover:bg-muted hover:text-foreground">Clear</button>
        )}
        {items.length > 0 && (
          <button
            onClick={() =>
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-background shadow-[0_1px_3px_0_rgba(0,0,0,0.04)]">
        <div className="flex items-center border-b border-border bg-muted/40 px-4 py-2">
          ...
          <div className="w-28 text-right text-[11px] font-semibold uppercase tracking-[0.06em] text-muted-foreground">Cost</div>
          <div className="w-28 text-right text-[11px] font-semibold uppercase tracking-[0.06em] text-muted-foreground">Profit</div>
          <div className="w-16 text-right text-[11px] font-semibold uppercase tracking-[0.06em] text-muted-foreground">Margin</div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="mt-1 text-[12px] text-muted-foreground">No completed sales found for the selected period</p>
          </div>
        ) : (
          <div className="divide-y divide-border">
            {items.map((item, i) => (
              <div key={item.productId} className="flex items-center px-4 py-2.5 transition-colors hover:bg-accent/40">
                <div className="w-8 text-[11px] tabular-nums text-muted-foreground">{i + 1}</div>
                ...
                <div className="w-20">
                  <span className={cn("inline-flex rounded-md px-1.5 py-0.5 text-[9px] font-semibold uppercase", CATEGORY_COLORS[item.category] ?? "bg-muted text-muted-foreground")}>
                    {item.category.replace("_", " ")}
                  </span>
                </div>
                <div className="w-16 text-right text-[12px] tabular-nums text-foreground">{item.unitsSold}</div>
                <div className="w-28 text-right text-[12px] tabular-nums font-medium text-foreground">PHP {fmt(item.totalRevenue)}</div>
                <div className="w-28 text-right text-[12px] tabular-nums text-muted-foreground">PHP {fmt(item.totalCost)}</div>
                <div className="w-28 text-right text-[12px] tabular-nums font-medium text-emerald-600">PHP {fmt(item.grossProfit)}</div>
