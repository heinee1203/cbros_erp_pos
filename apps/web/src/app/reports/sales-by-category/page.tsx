...
import { Grid3x3, DollarSign, Hash, TrendingUp, Download } from "lucide-react";
...
export default function SalesByCategoryPage() {
  ...
  const categories = reportQuery.data?.data ?? [];
  ...
  const maxRevenue = Math.max(...categories.map(c => parseFloat(c.totalRevenue)), 1);
    ...
    <div className="mx-auto flex h-full max-w-5xl flex-col">
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-background shadow-[0_1px_3px_0_rgba(0,0,0,0.04)]">
        <div className="flex items-center border-b border-border bg-muted/40 px-4 py-2">
          <div className="w-36 text-[11px] font-semibold uppercase tracking-[0.06em] text-muted-foreground">Category</div>
          <div className="flex-1 text-[11px] font-semibold uppercase tracking-[0.06em] text-muted-foreground">Revenue Bar</div>
          ...
          <div className="w-16 text-right text-[11px] font-semibold uppercase tracking-[0.06em] text-muted-foreground">SKUs</div>
        </div>
        ...
        {reportQuery.isLoading ? (
          ...
          <div className="divide-y divide-border">
            {categories.map((cat) => {
              ...
              return (
                <div key={cat.category} className="flex items-center px-4 py-3 transition-colors hover:bg-accent/40">
                  <div className="w-36">
                    <span className={cn("inline-flex rounded-md px-2 py-0.5 text-[10px] font-semibold uppercase", CATEGORY_COLORS[cat.category] ?? "bg-muted text-muted-foreground")}>
                      {cat.category.replace("_", " ")}
                    </span>
                  </div>
                  <div className="flex-1 pr-4">
                    <div className="h-2 w-full rounded-full bg-muted">
                      <div className="h-2 rounded-full bg-primary/40" style={{ width: `${pct}%` }} />
                    </div>
                  </div>
                  <div className="w-16 text-right text-[12px] tabular-nums text-foreground">{cat.unitsSold}</div>
                  <div className="w-28 text-right text-[12px] tabular-nums font-medium text-foreground">PHP {fmt(cat.totalRevenue)}</div>
                  <div className="w-28 text-right text-[12px] tabular-nums font-medium text-emerald-600">PHP {fmt(cat.grossProfit)}</div>
                  <div className="w-16 text-right text-[12px] tabular-nums font-semibold text-foreground">{cat.marginPct}%</div>
                  <div className="w-16 text-right text-[12px] tabular-nums text-muted-foreground">{cat.uniqueProducts}</div>
                </div>
              );
            })}
          </div>
